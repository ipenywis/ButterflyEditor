...
export default class DropDown extends React.Component < DropDownProps > {
  ...
  render() {
    ...
    const childsArr = React
      .Children
      .toArray(this.props.children);
    if (isEqual(childsArr, Array(childsArr.length).fill(undefined)) || isEqual(childsArr, Array(childsArr.length).fill(null)) || isEqual(childsArr, Array(childsArr.length).fill(""))) 
      return null;
    ...
    const isActive = (label : string, alise?: string) => {
      ...
      // active or not
      if (this.props.isItemActive) 
        return this.props.isItemActive(label);
      
      // Using alise when label doesn't match the currently active block's type for ex:
      // H5 != header-five
      if (alise) 
        return includes(activeItems, alise);
      let active = false;
    ...
    return (
      <SafeWrapper>
        ...
        {renderContainer == "select" && (
          <select
            className={this.props.className + (isDisabled
            ? " disabled"
            : "")}
            defaultValue="Header"
        ...
        {renderContainer == "button" && (
          <OutsideClickHandler
            onOutsideClick={() => this.props.outsideClickCanClose
            ? this.setState({isOpen: false})
            : null}>
            <button
              className={"btn-dropDown " + this.props.className + (isDisabled
              ? " disabled"
              : "")}
              style={{
              width: "50px",
              ...
              <span style={{
                ...
                <Icon icon={"minimalArrow"} size={iconFontSize - 2} rotation={IIconRotate.TOP}/>
              </span>
              <div
                style={{
                display: "flex",
                ...
                {this.state.isOpen && (
                  <div className="btn-dropDown-container ip-scrollbar">
                    {React
                      .Children
                      .map(this.props.children, (child : any, idx) => {
                        if (child) 
                          return (
                            <div
                              className={"dropDown-item " + (isActive(child.toString())
                              ? "toggle"
                              : "")}
                              key={idx}
                              onMouseDown={e => {
